...
class PointsController {
  async index(request: Request, response: Response) {
    ...
    .whereIn('point_items.item_id', parsedItems).where('city', String(city)).where('uf', String(uf))
    .distinct().select('points.*');

    const serializedPoints = points.map((point: any) => {
      return {
        ...point,
        image_url: `http://192.168.10.7:3333/uploads/${point.image}`
      };
    })
  ...
  async create(request: Request, response: Response) {
    ...
    const point_items = items.split(',').map((item: string) => Number(item.trim()))
      .map((item_id: number) => {
      return {
        item_id,
        point_id
